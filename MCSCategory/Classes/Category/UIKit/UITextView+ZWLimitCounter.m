...
- (void)zwlimitCounter_swizzling_layoutSubviews {
    ...
    if (self.zw_limitCount) {
       ...
       // CGFloat x = CGRectGetMinX(self.frame)+self.layer.borderWidth;
        CGFloat x = CGRectGetMaxX(self.frame)-LabWidth-self.zw_labMargin;
        CGFloat y = CGRectGetMaxY(self.frame)-self.contentInset.bottom-self.zw_labMargin;
      ...
      //  self.zw_inputLimitLabel.frame = CGRectMake(x, y, width, height);
        self.zw_inputLimitLabel.frame = CGRectMake(x, y, LabWidth, height);
        [self.zw_inputLimitLabel setLayerRadius:self.zw_inputLimitLabel.height/2];
